import { BillboardComponent, CameraComponent, ComponentType, LightComponent, MeshRendererComponent, ShadowMapComponent, TransformComponent } from '../components'
...
export class RendererSystem {
  ...
  public render(scene: Scene, cameraId: EntityId) {
    ...
    scene.getComponents([ComponentType.TRANSFORM, ComponentType.CAMERA]).forEach(({ transform, camera }) => {
      this.gpuDataInterface.writeCameraBuffer(transform as TransformComponent, camera as CameraComponent, currentWidth, currentHeight)
    })
