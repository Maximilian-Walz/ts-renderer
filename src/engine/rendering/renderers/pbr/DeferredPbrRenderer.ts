...
export class DeferredPbrRenderer extends Renderer {
  ...
  private writeGBuffer(commandEncoder: GPUCommandEncoder, width: number, height: number, modelsData: ModelData[], camera: CameraComponent): void {
  ...
  private doShadingForLightType(renderPass: GPURenderPassEncoder, pipeline: GPURenderPipeline, lightsData: ShadowMapLightData[], camera: CameraComponent): void {
  ...
  private doShading(commandEncoder: GPUCommandEncoder, targetView: GPUTextureView, lightsData: ShadowMapLightData[], camera: CameraComponent): void {
