...
export class SunLightShadowMapper extends Renderer {
  ...
  private createPipeline(): GPURenderPipeline {
    return this.device.createRenderPipeline({
      ...
      layout: this.device.createPipelineLayout({
        bindGroupLayouts: [
          LightComponent.getBindGroupLayout(this.device),
          CameraComponent.getBindGroupLayout(this.device),
          TransformComponent.getBindGroupLayout(this.device),
        ],
      }),
      vertex: {
  ...
  public renderShadowMap(
    ...
    shadowCasters: ModelData[],
    { light, shadowMap }: ShadowMapLightData,
  ...
  public render(commandEncoder: GPUCommandEncoder, renderingData: RenderingData): RenderingData {
    ...
    const consideredLights = (
      scene.getComponents([
        ComponentType.TRANSFORM,
        ComponentType.LIGHT,
        ComponentType.SHADOW_MAP,
      ]) as ShadowMapLightData[]
    ).filter(({ light }) => light.castShadow && light.lightType == LightType.SUN)

    const activeCamera = scene.getEntity(cameraId).getComponent(CameraComponent)
    consideredLights.forEach((shadowMappingData) =>
      this.renderShadowMap(commandEncoder, shadowCasters, shadowMappingData, activeCamera)
    )
