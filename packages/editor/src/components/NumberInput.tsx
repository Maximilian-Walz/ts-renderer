...
import React, { FocusEvent, RefObject, useEffect, useRef, useState } from "react"
...
export function NumberInput({
  ...
  const numberInputRef: RefObject<HTMLDivElement> = useClickAway(() => {
    setEditing(false)
  ...
  const wheelTimeout = useRef<number | boolean>(0)
      ...
      <div className="join join-horizontal mr-auto bg-gray-700" ref={inputRef}>
        ...
        <div className="relative">
          {labelContained && !editing && (
            <span className="label-text absolute bottom-0 top-0 self-center pl-1">{label}</span>
          )}
          <input
            type="text"
