import { Entity, LightComponent } from "@my/engine"
  ...
  return (
    <ComponentViewer title="Light" icon={<LuLightbulb />} contentKey={entity.entityId}>
      <div className="self-end">
        <NumberInput
          label="Power"
          initialValue={light.power}
          minValue={0}
          onChange={(value) => (light.power = value)}
        />
      </div>
      <VectorInput
        label="Color"
        ...
        minValue={0}
        maxValue={1}
        targetValue={light.color}
      />
      <input
        type="checkbox"
        className="checkbox"
        defaultChecked={light.castShadow as boolean}
        onChange={(event) => (light.castShadow = event.target.checked)}
      />
    </ComponentViewer>
  )
}
